...
def get_today_top_user(delta_day=1, top_count=30):
    ...
    excel_header = [u'uid', u'注册时间', u'充值额度', u'充值册数', u'消费次数', u'消费人次', u'中奖次数', u'中奖金额']
...
def get_today_new(delta_day=1):
    ...
    for item in items:
        ...
        today_chn_dict.setdefault(chn, {'bingo_list':[], 'pay_one_list':[], 'recharge_list':[], 'recharge_count': 0, 'recharge_amount': 0, 'user_count': 0})
    ...
    for pay_type, pay_price in pay_dict.items():
        html_str += u'支付方式: %s  支付金额: %s<br />' % (PAY_TYPE.get_label(pay_type), pay_price)
    ...
    for chn,info in chn_dict.items():
        today_info = today_chn_dict.get(chn) or {'user_count': 0, 'pay_one_list': [], 'recharge_list': [], 'recharge_count': 0,
            'recharge_amount': 0}
	html_str += u'<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>' % (date_str, chn,
    ...
    for chn, today_info in today_chn_dict.items():
    ...
    html_str += u'<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>' % (date_str, u'总计',
...
def get_yesterday_active():
    ...
    for chn, info in chn_dict.items():
	    html_str += u'渠道：%s, 新增人数:%s, 活跃人数:%s, 次日留存:%s%% <br />' % (chn, info['user_count'],
		len(info['bingo_list']), round(float(len(info['bingo_list']))*100/info['user_count'], 2))
    html_str += u'总计, 新增人数:%s, 活跃人数:%s, 次日留存:%s%% <br />' % (total_user_count, len(total_bingo_list),
        round((float(len(total_bingo_list))/total_user_count)*100, 2))
    return html_str
...
if __name__ == "__main__":
    ...
    if cmd == 'new':
        ...
        html_str += '</body></html>'
        # print html_str
        mail_sender.send("今日新增&&前日留存", html_str)
    if cmd == 'top':
        get_today_top_user()
    if cmd == 'history':
        html_str = history_data(last_days=18)
        mail_sender.send("[03/01 -- 03/17]历史数据", html_str)
    if cmd == "pay":
        dd = sys.argv[2]
