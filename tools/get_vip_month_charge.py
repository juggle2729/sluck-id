...
month_day = datetime.datetime.utcnow().strftime('%Y-%m-%d')
start_time = '%s 00:00:00' % datetime.date(month_day.year,month_day-1, 1)
end_time = '%s 00:00:00' % datetime.date(month_day.year, month_day.month, 1)
...
if __name__ == '__main__':
    ...
    table_title = ["uid", "phone", "all_pay_amount", "bor_time", "bor_amount"]
    file_path = redirect_to_file(result_info_list, table_title, u'month_pay_user%s.xlsx' % end_time.strftime('%y-%m-%d %HH:%MM:%S'))
    TOOL_MAIL_SENDER.send(u"%s--每月支付用户信息"%start_time, "to see the attachment", attachments=[file_path])
