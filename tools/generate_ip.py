...
_START_END_LIST = [
    ...
    ('113.22.0.0', '113.22.255.255'),
    ('113.23.0.0', '113.23.127.255'),
    ...
    ('183.80.0.0', '183.80.255.255'),
    ('183.81.0.0', '183.81.127.255'),
    ...
    ('203.162.32.0', '203.162.63.255'),
    ('203.162.64.0', '203.162.127.255'),
    ('203.162.128.0', '203.162.143.255'),
    ('203.162.144.0', '203.162.159.255'),
    ('203.162.160.0', '203.162.191.255'),
...
def _get_part(ip, num):
    return ip.split('.')[num - 1]


def generate_ip(start, end):
    ip_list = []
    for index_1 in xrange(int(_get_part(start, 1)), int(_get_part(end, 1)) + 1):
        for index_2 in xrange(int(_get_part(start, 2)), int(_get_part(end, 2)) + 1):
            for index_3 in xrange(int(_get_part(start, 3)), int(_get_part(end, 3)) + 1):
                for index_4 in xrange(int(_get_part(start, 4)), int(_get_part(end, 4)) + 1):
                    ip = '%s.%s.%s.%s' % (index_1, index_2, index_3, index_4)
                    ip_list.append(ip)
    return ip_list
...
def generate(count):
    ip_list = []
    for pair in _START_END_LIST:
        l = generate_ip(pair[0], pair[1])
        ip_list += l
    random.shuffle(ip_list)
    return ip_list[0: count]


if __name__ == '__main__':
    for ip in generate(400):
        print ip
