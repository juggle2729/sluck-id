...
COINS = [(0.91, 1.00), (4.55, 5.00), (9.09, 10.00), (45.45, 50.00),
         (90.91, 100.00), (181.82, 200.00), (454.55, 500.00)]

for wrong_price, origin_price in COINS:
    add_value = origin_price - wrong_price
    pays = Pay.query.filter(Pay.pay_type == 19).filter(Pay.status == 2).filter(
        Pay.price == wrong_price).all()
    print "原始金额： %s，实际金额： %s，差额： %s，处理数量： %s" % (origin_price, wrong_price,
                                                 add_value, len(pays))
    for pay in pays:
        ...
        if pay.user_id is not None:
            ...
            if account is not None:
                ...
                print 'UID: %s, Phone: %s, 现余额：%s, 更新后余额： %s, 增加值：%s' % (
                    account.id, account.phone, old_balance, account.balance,
                    Decimal(add_value).quantize(Decimal('0.01')))
                account.save()
                pay.status = 3
                pay.save()
