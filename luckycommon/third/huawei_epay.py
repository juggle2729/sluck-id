...
def get_response_(body_data, url, method="POST"):
    ...
    'X-WSSE': 'UsernameToken Username="%s", PasswordDigest="%s", Nonce="%s", Created="%s"'%(username, passworddigiest, nonce, created),
    ...
    if method == "GET":
        ...
        return response
    try:
        response = requests.post(url, data=json.dumps(body_data), headers=headers, allow_redirects=False, timeout=5.0)
        return response
    except requests.exceptions.Timeout:
        _LOGGER.info("#HUAWEI PAY# requset %s connect time out, body %s" %(url, body_data))
    except Exception, e:
        _LOGGER.info("#HUAWEI PAY# requset %s , %s in %s request with body %s" %(e.__class__.__name__, e, url, body_data))
    return None
