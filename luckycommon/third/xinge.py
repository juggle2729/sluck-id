...
class TimeInterval(object):
    ...
    def _isValidTime(self, hour, minute):
        return isinstance(hour, int) and isinstance(minute,
                                                    int) and hour >= 0 and hour <= 23 and minute >= 0 and minute <= 59
...
def _BuildIosNotification(content, custom):
    msg = MessageIOS()
    msg.alert = content
    msg.custom = custom
    msg.sound = 'default'
...
def PushTokenAndroid(accessId, secretKey, title, content, custom, token):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleDevice(token, _BuildAndroidNotification(title, content, custom))


def PushAccountAndroid(accessId, secretKey, title, content, custom, account):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleAccount(0, account, _BuildAndroidNotification(title, content, custom))


def PushAllAndroid(accessId, secretKey, title, content, custom):
    x = XingeApp(accessId, secretKey)
    return x.PushAllDevices(0, _BuildAndroidNotification(title, content, custom))


def PushTagAndroid(accessId, secretKey, title, content, custom, tag):
    x = XingeApp(accessId, secretKey)
    return x.PushTags(0, (tag,), 'OR', _BuildAndroidNotification(title, content, custom))


def PushTokenIos(accessId, secretKey, content, custom, token, env):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleDevice(token, _BuildIosNotification(content, custom), env)


def PushAccountIos(accessId, secretKey, content, custom, account, env):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleAccount(0, account, _BuildIosNotification(content, custom), env)


def PushAllIos(accessId, secretKey, content, custom, env):
    x = XingeApp(accessId, secretKey)
    return x.PushAllDevices(0, _BuildIosNotification(content, custom), env)


def PushTagIos(accessId, secretKey, content, custom, tag, env):
    x = XingeApp(accessId, secretKey)
    return x.PushTags(0, (tag,), 'OR', _BuildIosNotification(content, custom), env)


if __name__ == '__main__':
    print PushAccountIos(2200194027, 'a24df2ea34f4ef67021a22a1ba41a032', 'python 处理 unicode 真烦',
                   {'command': '32#gogogo'}, '7346', XingeApp.ENV_DEV)
